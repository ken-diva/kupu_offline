...
def render_data(img_front, img_back, render_type):
    ...
    kupu.load_state_dict(
        torch.load("./model/model-btr-paper.pt", map_location=torch.device("cpu"))
    )
    ...
    for i in range(n_data_valid):
        ...
        out_ant, out_pos = kupu(x[0].reshape(inp_size), x[1].reshape(inp_size))
...
def crop(img_direction):
    ...
    # gambar = "https://storage.googleapis.com/kupu-364702.appspot.com/kendiva17%40gmail.com/20230623-192236/p2c/img_front_20230623-192236.png"

    # img = Image.open(io.BytesIO(urllib.request.urlopen(gambar).read()))
        ...
        for item in d:
            ...
            if item[2] in list(range(ic, ic + 1)):
                if ic == 0:
                    ...
                    if counter <= img_len / 2 and "_r" in bone:
                        new_image.append((109, 21, 193, 190))
                    elif counter > img_len / 2 and "_l" in bone:
                        new_image.append((109, 21, 193, 190))
                    else:
